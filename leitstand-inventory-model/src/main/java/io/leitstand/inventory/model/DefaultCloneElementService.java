...
public class DefaultCloneElementService implements CloneElementService{
	...
	private ElementId cloneElement(Element source, 
		...
		db.executeUpdate(prepare("INSERT INTO inventory.element (elementgroup_id, elementrole_id, platform_id, id, uuid, name, description, opstate, admstate, mgmtmac, serial, location, modcount, tscreated, tsmodified)"+
								 "SELECT elementgroup_id, elementrole_id, platform_id, ?, ?, ?, description, ?, ?, ?, ?, location, 0, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP "+
