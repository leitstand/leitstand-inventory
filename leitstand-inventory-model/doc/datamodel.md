...
### `element_metric` Table
The `element_metric` table stores which metrics can be sampled from a network element.
An element can provide an arbitrary number of metrics, 
and the same metric can be supplied by multiple elements.

#### Columns
| Column 				  | Type 		 | Description 							|
|:------------------------|:-------------|:-------------------------------------|
| ELEMENT_ID	 			  | INT8			 | Reference to the element record.		|
| METRIC_ID  			  | INT8			 | Reference to the metric record. 		|

#### Primary Key
The `ELEMENT_ID` and `METRIC_ID` columns form the primary key.

#### Foreign Keys
This table has two foreign keys:
- The `ELEMENT_ID` column refers to the `ID` column of the `element` table.
  This relation connects the metric to the element.
- The `METRIC_ID` column refers to the `ID` column of the `metric` table.
  This relation connects the element to the metric.
...
### `metric` Table
The `metric` table contains the general settings of all available metrics.

#### Columns
| Column      | Type 		  | Description 										 |
|:------------|:--------------|:-------------------------------------------------|
| ID		 	  | INT8          | Contiguous number as primary key. 				 |
| UUID		  | CHARACTER(36) | Unique metric ID.								 |
| NAME	 	  | VARCHAR(64)   | Unique metric name. 								 |
| UNIT   	  | VARCHAR(16)   | Metric base unit (e.g. °C, bps, rpm, %, MB) 		 |
| SCOPE		  | VARCHAR(16)   | Metric scope (e.g. physical interface, element). |
| DESCRIPTION | VARCHAR(1024) | Optional description of the metric. 				 |
| MODCOUNT	  | INT4			  | Modification counter.							 |
| TSMODIFIED  | TIMESTAMP	  | Last-modification timestamp.					     |
| TSCREATED   | TIMESTAMP	  | Creation timestamp.								 |
...
- `UUID`, the metric ID must be unique.
- `NAME`, the metric name must be unique.

### `metric_alertrule` Table
The `metric_alertrule` table contains alert rule settings per metric.
...
| ID			  | INT8			  | Contiguous number as primary key.   |
| METRIC_ID   | INT8          | Reference to the metric record.		|
| NAME		  | VARCHAR(64)   | Alert rule name.						|
| DESCRIPTION | VARCHAR(1024) | Optional alert rule description.	    |
| CATEGORY	  | VARCHAR(128)	  | Rule category.						|

The correlation ID allows to specify which rule fired an event.

#### Primary Key
The `ID` column forms the primary key.

#### Unique Constraints
The `METRIC_ID` and `NAME` columns form a unique key,
i.e. the rule name must be unique per metric.

#### Foreign Keys
This table has a single foreign key only.
The `METRIC_ID` column refers to the `ID` column in the `metric` table.
This relation assigns an alert rule to its metric. 

### `metric_alertrule_definition` Table
The `metric_alertrule_definition` table contains the alert rule definitions per alert rule.
This table allows to maintain an alert rule history.

#### Columns
| Column                 | Type 		   | Description						            |
|:-----------------------|:------------|:-----------------------------------|
| METRIC\_ALERTRULE_ID   | INT8        | Reference to alert rule record.	    |
| RULEID		             | VARCHAR(64) | Unique alert rule ID.			    |
| STATE		             | VARCHAR(16) | Rule state.                        |
| TYPE		             | VARCHAR(128)| Rule definition type.	      		|
| RULE		             | JSON        | The rule definition as JSON object.	|
| TSMODIFIED             | TIMESTAMP	   | Last-modification timestamp.		|

The following rule states exist:
- `CANDIDATE`, a candidate rule definition that is under evaluation.
- `ACTIVE`, the currently active rule definition.
- `SUPERSEDED`, an outdated rule definition.

The `RULEID` allows to specify which rule definition fired an event.

#### Primary Key
The `METRIC_ALERTRULE_ID` and `TSMODIFIED` columns form the primary key.

#### Unique Constraints
The value of the `RULEID` column must be unique,
i.e. the correlation ID is unique for all rule definitions.
...
### `metric_elementrole` Table
The `metric_elementrole` join table stores on which roles a metric is available.
...
#### Primary Key
The `METRIC_ID` and `ELEMENTROLE_ID` columns form the primary key.
...
### `metric_visualization` Table
The `metric_visualization` table stores metric visualization configurations per metric.

#### Columns
| Column      | Type          | Description 						    |
|:------------|:--------------|:------------------------------------|
| METRIC_ID   | INT8 		  | Reference to the metric record.     |
| NAME		  | VARCHAR(64)	  | Unique visualization name.		    |
| DESCRIPTION | VARCHAR(1024) | Optional visualization description. |
| CATEGORY    | VARCHAR(128)	  | Visualization category.             |
| TYPE		  | VARCHAR(128)	  | Visualization configuration type.   |
| CONFIG		  | JSON			  | The visualization configuration.		|

#### Primary Key
The `METRIC_ID` and `NAME` column form the primary key.

#### Foreign Keys
This tables has a single foreign key only.
The `METRIC_ID` column refers to the `ID` column of the `metric` table.
This relation assigns a visualization to its metric.
